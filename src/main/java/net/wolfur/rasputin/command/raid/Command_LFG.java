package net.wolfur.rasputin.command.raid;
        ...
        if (event.getTextChannel().getIdLong() == Main.getFileManager().getChannelFile().getChannel("lfg").getChannelId()) {
            ...
            if (args.length >= 3) {
                ...
                if (raidType != null) {
                    ...
                    if(event.getTextChannel().getIdLong() != raidType.getChannelId()) {
                        event.getTextChannel().sendMessage(new EmbedBuilder().setColor(Color.RED).setDescription("Diese Aktivität kann in diesem Channel nicht verwendet werden.").build()).queue(message -> {
