...
public class Command_Throne implements Command {
    ...
    private EmbedBuilder createEmbedBuilder(BungieUser targetUser, JsonObject shatteredThroneDataObject, List<JsonObject> shatteredThroneActivities) {
        JsonObject metricsData = shatteredThroneDataObject.getAsJsonObject("Response").getAsJsonObject("metrics").getAsJsonObject("data").getAsJsonObject("metrics");

        int completions = metricsData.getAsJsonObject("1339818929").getAsJsonObject("objectiveProgress").get("progress").getAsInt();
        boolean flawless = targetUser.ownsTriumph(1178448425L);
        boolean soloFlawless = targetUser.ownsTriumph(3205009787L);
        ...
        if(shatteredThroneActivities.size() > 0) {
            ...
            bungieError = true;
            Logger.warning("No 'Shattered Throne' activities found for " + targetUser.getUser().getName() + ".", true);
        }

        EmbedBuilder embedBuilder = new EmbedBuilder()
                .setColor(Color.MAGENTA)
                .setTitle("Statistics > Shattered Throne: " + targetUser.getUser().getName())
