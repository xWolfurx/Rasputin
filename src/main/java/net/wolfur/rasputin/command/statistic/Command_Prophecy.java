...
public class Command_Prophecy implements Command {
        ...
        if(Main.getFileManager().getChannelFile().isCommandChannel(event.getTextChannel().getIdLong())) {
            ...
            if (bungieUser.isRegistered()) {
                if(args.length == 0) {
                    ...
                    JsonObject prophecyDataObject = bungieUser.getProfile(ComponentType.METRICS);
                    List<JsonObject> dungeonActivities = bungieUser.getActivityHistory(DestinyActivityModeType.DUNGEON);

                    List<JsonObject> prophecyActivities = dungeonActivities.stream().filter(activity -> activity.getAsJsonObject("activityDetails").get("directorActivityHash").getAsLong() == 1077850348L).collect(Collectors.toList());

                    event.getTextChannel().sendMessage(this.createEmbedBuilder(bungieUser, prophecyDataObject, prophecyActivities).build()).complete();
                    ...
                    if (targetUser != null) {
                        ...
                        if (targetBungieUser.isRegistered()) {
                            ...
                            event.getTextChannel().sendMessage(this.createEmbedBuilder(targetBungieUser, trialsDataObject, prophecyActivities).build()).complete();
                        } else {
                    ...
                    event.getTextChannel().sendMessage(new EmbedBuilder().setColor(Color.RED).setDescription("Verwendung: .Prophecy [@Player]").build()).queue(message -> {
                        message.delete().queueAfter(15, TimeUnit.SECONDS);
                    });
                }
            } else {
    ...
    private EmbedBuilder createEmbedBuilder(BungieUser targetUser, JsonObject prophecyDataObject, List<JsonObject> prophecyActivities) {
        JsonObject metricsData = prophecyDataObject.getAsJsonObject("Response").getAsJsonObject("metrics").getAsJsonObject("data").getAsJsonObject("metrics");

        int completions = metricsData.getAsJsonObject("352659556").getAsJsonObject("objectiveProgress").get("progress").getAsInt();
        boolean flawless = targetUser.ownsTriumph(2010041484L);
        boolean soloFlawless = targetUser.ownsTriumph(3191784400L);
        ...
        if(prophecyActivities.size() > 0) {
            for(JsonObject activity : prophecyActivities) {
                try {
            ...
            bungieError = true;
            Logger.warning("No 'Prophecy' activities found for " + targetUser.getUser().getName() + ".", true);
        }

        EmbedBuilder embedBuilder = new EmbedBuilder()
                .setColor(Color.MAGENTA)
                .setTitle("Statistics > Prophecy: " + targetUser.getUser().getName())
