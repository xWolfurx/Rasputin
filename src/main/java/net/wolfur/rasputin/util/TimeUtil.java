...
public class TimeUtil {
    ...
    public static String timeToString(long time, boolean shorten) {
        ...
        if (seconds >= 86400L) {
            ...
            msg = msg + days + (days == 1L ? shorten ? "d " : " Tag, " : shorten ? "d " : " Tage, ");
            seconds %= 86400L;
        }
        if (seconds >= 3600L) {
            long hours = seconds / 3600L;
            msg = msg + hours + (hours == 1L ? shorten ? "h " : " Stunde, " : shorten ? "h " : " Stunden, ");
            seconds %= 3600L;
        }
        if (seconds >= 60L) {
            long minutes = seconds / 60L;
            msg = msg + minutes + (minutes == 1L ? shorten ? "m " : " Minute, " : shorten ? "m " : " Minuten, ");
            seconds %= 60L;
        }
        if (seconds > 0L) {
            msg = msg + seconds + (seconds == 1L ? shorten ? "s " : " Sekunde, " : shorten ? "s " : " Sekunden, ");
        }
        if (!msg.isEmpty()) {
